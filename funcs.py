...
from admin import BOT, TIMES, ADMINISTRATORS, CURRENCIES
...
async def update_buttons(user_id, today):
    try:
        await BOT.send_message(chat_id=user_id, text=f"Обновление бота от {today}", disable_notification=True,
                               reply_markup=reply_builder.as_markup(resize_keyboard=True))
        # await message.delete()
    except Exception as e:
        logging.error(f"Ошибка при обновлении кнопок для {user_id}: {e}")
...
async def update_bot():
    ...
    for i in range(0, len(users), batch):
        users_batch = users[i:i + batch]
        tasks = [update_buttons(user, today) for user in users_batch]
...
async def add(message):
    ...
    for curr in CURRENCIES:
        builder.add(InlineKeyboardButton(text=curr, callback_data=f"{curr}_add"))
    builder.adjust(3)
...
async def get_user_currencies_list(message):
    currencies_list = db.user_currencies(message.from_user.id)
    if not currencies_list:
        await message.answer("У вас не выбрано ни одной валюты")
    else:
        result = "\n".join(f"{i + 1}. {curr}" for i, curr in enumerate(currencies_list))
        await message.answer(f"Ваши выбранные валюты:\n{result}")


def create_currency_data(currency: str, price: float, percentage: float) -> str:
    format_price = '{:,.4f}'.format(price)
    percentage = round(float(percentage), 2)
    chart_emoji = emoji.emojize(":chart_increasing:" if percentage > 0 else ":chart_decreasing:")
    percentage_str = f"+{percentage}" if percentage > 0 else str(percentage)
    return f"{currency}: {format_price} USD. {chart_emoji} {percentage_str}% за 24 ч.\n\n"


def get_now_currencies(message: Message) -> str:
    currency_list = db.user_currencies(message.from_user.id)
    ...
    try:
        ...
        for e in data['data']:
            for curr in currency_list:
                if e['symbol'] == curr:
                    curr_data = create_currency_data(
                        curr,
                        e['quote']['USD']['price'],
                        e['quote']['USD']['percent_change_24h']
                    )
                    result += curr_data
                    db.add_currency_price(curr, e['quote']['USD']['price'])
                    break
...
def currencies_prices(users_list: List[str]) -> List[str]:
    result_array = []
    if not users_list:
        return []
    session.headers.update(headers)
    ...
    for user in users_list:
        ...
        try:
            ...
            logging.error(f"Ошибка при обработке валют пользователя {user}: {e}")
    return result_array
...
async def send_push_messages(users: List[str], currencies: List[str]) -> None:
    # for user, currency in zip(users, currencies):
    #     await bot.send_message(user, currency)
    batch = 20
    delay = 1
    for i in range(0, len(users), batch):
        batch_users = users[i:i + batch]
        batch_currencies = currencies[i:i + batch]
        tasks = [BOT.send_message(user, currency) for user, currency in zip(batch_users, batch_currencies)]
        await asyncio.gather(*tasks)
        await asyncio.sleep(delay)
